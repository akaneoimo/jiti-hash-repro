...
| `vp-layout-minimal/` | Standalone repro of vp's install layout (`packages/<name>#<installId>/`): same module fails in the vp layout, works when `%23`-escaped or under `node_modules/<name>/`. No dependencies. README includes a real-environment check (vp vs npm, same package). |
